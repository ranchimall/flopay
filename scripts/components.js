    ...
    class extends HTMLElement {
      ...
      attributeChangedCallback(t, n, o) {
        if ("value" === t) {
          ...
          (n && n.length) || (this.copyContent.textContent = o);
        }
      }
      disconnectedCallback() {
    ...
    class extends HTMLElement {
      ...
      _checkValidity() {
        this.submitButton &&
          ...
          this._requiredElements.forEach(([e, t]) => {
            ((!e.disabled && t && !e.isValid) || (!t && !e.checkValidity())) &&
              this.invalidFieldsCount++;
          }),
      ...
      handleKeydown(e) {
        if ("Enter" === e.key && e.target.tagName.includes("INPUT"))
          if (0 === this.invalidFieldsCount)
            ...
            for (const [e, t] of this._requiredElements) {
              if (t ? !e.isValid : !e.checkValidity()) {
                (e?.shadowRoot?.lastElementChild || e).animate(
                  [
      ...
      elementsChanged() {
        ...
        ].map((e) => [e, e.tagName.includes("-")])),
          (this._requiredElements = this.formElements.filter(([e]) =>
      ...
      connectedCallback() {
        const e = this.debounce(this.elementsChanged, 100);
          ...
          (this.mutationObserver = new MutationObserver((t) => {
            t.forEach((t) => {
              (("childList" === t.type &&
                [...t.addedNodes].some(
                  (e) =>
                    1 === e.nodeType && e.querySelector(this.supportedElements)
                )) ||
                [...t.removedNodes].some(
                  (e) =>
                    1 === e.nodeType && e.querySelector(this.supportedElements)
      ...
      attributeChangedCallback(e, t, i) {
        "skip-submit" === e && (this.skipSubmit = null !== i);
      }
      disconnectedCallback() {
...
(smInput.innerHTML =
  ...
  customElements.define(
    ...
    class extends HTMLElement {
      constructor() {
        super(),
          ...
          (this.handleDatalistNavigation =
            this.handleDatalistNavigation.bind(this)),
      ...
      set value(t) {
        t !== this.input.value && ((this.input.value = t), this.checkInput());
      }
      get placeholder() {
      ...
      showError() {
        (this.feedbackText.className = "feedback-text error"),
          (this.feedbackText.innerHTML = ` <svg class="status-icon status-icon--error" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path fill="none" d="M0 0h24v24H0z"/><path d="M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm-1-7v2h2v-2h-2zm0-8v6h2V7h-2z"/></svg> ${this._errorText}`);
      }
      set helperText(t) {
        this._helperText = t;
      }
        ...
        if ("" !== this.input.value) {
          ...
          return (
            this.validationFunction &&
              (e = Boolean(this.validationFunction(this.input.value))),
            t && e
              ? ((this.feedbackText.className = "feedback-text success"),
                (this.feedbackText.textContent = ""))
              : this._errorText && this.showError(),
            t && e
          );
        }
      }
      ...
      searchDatalist(t) {
        const e = this.datalist.filter((e) =>
          e.toLowerCase().includes(t.toLowerCase())
        );
        if (
          (e.sort((e, n) => {
            const i = e.toLowerCase().indexOf(t.toLowerCase()),
              s = n.toLowerCase().indexOf(t.toLowerCase());
            return i - s;
          }),
          e.length)
        ) {
          if (this.optionList.children.length > e.length) {
            const t = this.optionList.children.length - e.length;
            for (let e = 0; e < t; e++)
              this.optionList.removeChild(this.optionList.lastChild);
          }
          e.forEach((t, e) => {
            if (this.optionList.children[e])
              this.optionList.children[e].textContent = t;
            else {
              const e = document.createElement("li");
              (e.textContent = t),
                e.classList.add("datalist-item"),
                e.setAttribute("tabindex", "0"),
                this.optionList.appendChild(e);
            }
          }),
      ...
      allowOnlyNum(t) {
        1 === t.key.length &&
          ((("." !== t.key ||
            (!t.target.value.includes(".") && 0 !== t.target.value.length)) &&
      ...
      handleOptionClick(t) {
        (this.input.value = t.target.textContent),
          this.optionList.classList.add("hidden"),
          this.input.focus();
      }
      handleInputNavigation(t) {
        "ArrowDown" === t.key
          ? (t.preventDefault(),
      ...
      attributeChangedCallback(t, e, n) {
        if (e !== n)
          switch (
            (this.reflectedAttributes.includes(t) &&
              (this.hasAttribute(t)
                ? this.input.setAttribute(
                    t,
                    this.getAttribute(t) ? this.getAttribute(t) : ""
                  )
                : this.input.removeAttribute(t)),
            t)
          ) {
            case "placeholder":
              (this.label.textContent = n), this.setAttribute("aria-label", n);
              break;
            case "value":
              this.checkInput();
              break;
  ...
  customElements.define(
    ...
    class extends HTMLElement {
      ...
      createNotification(n, t = {}) {
        const { pinned: e = !1, icon: i = "", action: o } = t,
          r = document.createElement("div");
        (r.id = this.randString(8)),
          (r.className = `notification ${e ? "pinned" : ""}`);
        const a = document.createElement("div");
        (a.className = "icon-container"), (a.innerHTML = i);
        const s = document.createElement("output");
        if (((s.textContent = n), r.append(a, s), o)) {
          const n = document.createElement("button");
          (n.className = "action"),
            (n.innerText = o.label),
            n.addEventListener("click", o.callback);
        }
        if (e) {
          const n = document.createElement("button");
          (n.className = "close"),
      ...
      push(n, t = {}) {
        const e = this.createNotification(n, t);
        return (
          this.isLandscape
            ? this.notificationPanel.append(e)
            : this.notificationPanel.prepend(e),
          this.notificationPanel.animate(
            [
              {
                transform: `translateY(${this.isLandscape ? "" : "-"}${
                  e.clientHeight
                }px)`,
              },
              { transform: "none" },
          ...
          (e.animate(
            ...
            n.target.commitStyles(), n.target.cancel();
          }),
          e.querySelector(".action") &&
            e
              .querySelector(".action")
              .addEventListener("click", t.action.callback),
          e.id
        );
      }
      removeNotification(n, t = "left") {
        if (!n) return;
        const e = "left" === t ? "-" : "+";
        n.animate(
          [
            ...
            {
              transform: `translateX(calc(${e}${Math.abs(
                this.currentX
              )}px ${e} 1rem))`,
              opacity: "0",
            },
      ...
      remove(n) {
        const t = this.notificationPanel.querySelector(`#${n}`);
        t && this.removeNotification(t);
      }
      clearAll() {
        Array.from(this.notificationPanel.children).forEach((n) => {
          this.removeNotification(n);
        });
      }
      handlePointerMove(n) {
        (this.currentX = n.clientX - this.startX),
          (this.currentTarget.style.transform = `translateX(${this.currentX}px)`);
      }
      handleOrientationChange(n) {
        (this.isLandscape = n.matches), n.matches;
      }
      connectedCallback() {
        this.handleOrientationChange(this.mediaQuery),
          ...
          this.notificationPanel.addEventListener("pointerdown", (n) => {
            n.target.closest(".close")
              ? this.removeNotification(n.target.closest(".notification"))
              : n.target.closest(".notification") &&
                ((this.swipeThreshold =
                  n.target.closest(".notification").getBoundingClientRect()
                    .width / 2),
                (this.currentTarget = n.target.closest(".notification")),
                this.currentTarget.setPointerCapture(n.pointerId),
          ...
          this.notificationPanel.addEventListener("pointerup", (n) => {
            (this.endX = n.clientX),
              (this.endY = n.clientY),
              ...
              this.notificationPanel.releasePointerCapture(n.pointerId),
              (this.currentX = 0);
          });
        const n = new MutationObserver((n) => {
          n.forEach((n) => {
            "childList" === n.type &&
              n.addedNodes.length &&
              !n.addedNodes[0].classList.contains("pinned") &&
              setTimeout(() => {
                this.removeNotification(n.addedNodes[0]);
              }, 5e3);
          });
        });
        n.observe(this.notificationPanel, { childList: !0 });
      }
      disconnectedCallback() {
...
(smPopup.innerHTML = ` <style> *{ padding: 0; margin: 0; -webkit-box-sizing: border-box; box-sizing: border-box; } :host{ position: fixed; display: -ms-grid; display: grid; z-index: 10; --width: 100%; --height: auto; --min-width: auto; --min-height: auto; --backdrop-background: rgba(0, 0, 0, 0.6); --border-radius: 0.8rem 0.8rem 0 0; } .popup-container{ display: -ms-grid; display: grid; position: fixed; top: 0; bottom: 0; left: 0; right: 0; place-items: center; z-index: 10; touch-action: none; } :host(.stacked) .popup{ -webkit-transform: scale(0.9) translateY(-2rem) !important; transform: scale(0.9) translateY(-2rem) !important; } .backdrop{ position: absolute; top: 0; bottom: 0; left: 0; right: 0; background: var(--backdrop-background); -webkit-transition: opacity 0.3s; -o-transition: opacity 0.3s; transition: opacity 0.3s; } .popup{ display: -webkit-box; display: -ms-flexbox; display: flex; -webkit-box-orient: vertical; -webkit-box-direction: normal; flex-direction: column; position: relative; -ms-flex-item-align: end; align-self: flex-end; -webkit-box-align: start; -ms-flex-align: start; align-items: flex-start; width: var(--width); min-width: var(--min-width); height: var(--height); min-height: var(--min-height); max-height: 90vh; border-radius: var(--border-radius); background: rgba(var(--background-color, (255,255,255)), 1); -webkit-box-shadow: 0 -1rem 2rem #00000020; box-shadow: 0 -1rem 2rem #00000020; } .container-header{ display: -webkit-box; display: flex; width: 100%; touch-action: none; -webkit-box-align: center; -ms-flex-align: center; align-items: center; } .popup-top{ display: -webkit-box; display: flex; width: 100%; } .popup-body{ display: -webkit-box; display: flex; -webkit-box-orient: vertical; -webkit-box-direction: normal; -ms-flex-direction: column; flex-direction: column; -webkit-box-flex: 1; -ms-flex: 1; flex: 1; width: 100%; padding: var(--body-padding, 1.5rem); overflow-y: auto; } .hide{ display:none; } @media screen and (min-width: 640px){ :host{ --border-radius: 0.5rem; } .popup{ -ms-flex-item-align: center; -ms-grid-row-align: center; align-self: center; border-radius: var(--border-radius); height: var(--height); -webkit-box-shadow: 0 3rem 2rem -0.5rem #00000040; box-shadow: 0 3rem 2rem -0.5rem #00000040; } } @media screen and (max-width: 640px){ .popup-top{ -webkit-box-orient: vertical; -webkit-box-direction: normal; flex-direction: column; -webkit-box-align: center; align-items: center; } .handle{ height: 0.3rem; width: 2rem; background: rgba(var(--text-color, (17,17,17)), .4); border-radius: 1rem; margin: 0.5rem 0; } } @media (any-hover: hover){ ::-webkit-scrollbar{ width: 0.5rem; } ::-webkit-scrollbar-thumb{ background: rgba(var(--text-color, (17,17,17)), 0.3); border-radius: 1rem; &:hover{ background: rgba(var(--text-color, (17,17,17))), 0.5); } } } </style> <div class="popup-container hide" role="dialog"> <div part="backdrop" class="backdrop"></div> <div part="popup" class="popup"> <div part="popup-header" class="popup-top"> <div class="handle"></div> <slot name="header"></slot> </div> <div part="popup-body" class="popup-body"> <slot></slot> </div> </div> </div> `),
  customElements.define(
    ...
    class extends HTMLElement {
      ...
      animateTo(t, e, i) {
        let s = t.animate(e, { ...i, fill: "both" });
        return (
          s.finished.then(() => {
            s.commitStyles(), s.cancel();
          }),
          s
        );
      }
      ...
      setStateOpen() {
        if (!this.isOpen || this.offset) {
          ...
          this.animateTo(
            this.dialogBox,
            [
              { opacity: this.offset ? 1 : 0, transform: t },
      ...
      show(t = {}) {
        let { pinned: e = !1, payload: i } = t;
        if (this.isOpen) return;
        let s = { duration: 300, easing: "ease" };
        return (
          (this.payload = i),
          popupStack.push({ popup: this, permission: e }),
          ...
          (this.isOpen = !0),
          setTimeout(() => {
            let t = this.autoFocus || this.focusable?.[0] || this.dialogBox;
            t && (t.tagName.includes("-") ? t.focusIn() : t.focus());
          }, 0),
          ...
          {
            opened: new Promise((t) => {
              this.resolveOpen = t;
            }),
            closed: new Promise((t) => {
              this.resolveClose = t;
            }),
          }
        );
      ...
      hide(t = {}) {
        let { payload: e } = t,
          i = { duration: 150, easing: "ease" };
        this.backdrop.animate([{ opacity: 1 }, { opacity: 0 }], i),
          this.animateTo(
            this.dialogBox,
            [
              {
            ...
            this.popupContainer.classList.add("hide"),
              ...
              this.forms.length && this.forms.forEach((t) => t.reset()),
              this.dispatchEvent(
                new CustomEvent("popupclosed", {
                  bubbles: !0,
                  composed: !0,
                  detail: { payload: e || this.payload },
                })
              ),
              this.resolveClose(e || this.payload),
              (this.isOpen = !1);
          }),
          popupStack.pop(),
      ...
      handleTouchMove(t) {
        this.touchStartY < t.changedTouches[0].clientY &&
          ((this.offset = t.changedTouches[0].clientY - this.touchStartY),
      ...
      detectFocus(t) {
        if ("Tab" === t.key && this.focusable.length) {
          if (!this.firstFocusable) {
            for (let e = 0; e < this.focusable.length; e++)
              if (!this.focusable[e].disabled) {
                this.firstFocusable = this.focusable[e];
                break;
              }
          }
            ...
            t.forEach((t) => {
              if (t.contentBoxSize) {
                let e = Array.isArray(t.contentBoxSize)
                  ? t.contentBoxSize[0]
                  : t.contentBoxSize;
                this.threshold = 0.3 * e.blockSize.height;
              } else this.threshold = 0.3 * t.contentRect.height;
            });
          })),
          (this.mutationObserver = new MutationObserver((t) => {
            this.updateFocusableList();
      ...
      attributeChangedCallback(t) {
        "open" === t && this.hasAttribute("open") && this.show();
      }
    }
  );
      ...
      connectedCallback() {
        ...
        this.shadowRoot.querySelector("slot").addEventListener("slotchange", t),
          ...
          new IntersectionObserver((t, e) => {
            t.forEach((t) => {
              if (t.isIntersecting) {
                const t = this.selection.getBoundingClientRect().left;
  ...
  customElements.define(
    ...
    class extends HTMLElement {
      constructor() {
        super(),
          this.attachShadow({ mode: "open" }).append(
            smTextarea.content.cloneNode(!0)
          ),
      ...
      set value(e) {
        this.setAttribute("value", e), this.fireEvent();
      }
      get disabled() {
        return this.hasAttribute("disabled");
      }
      ...
      attributeChangedCallback(e, t, n) {
        this.reflectedAttributes.includes(e)
          ? this.hasAttribute(e)
            ? this.textarea.setAttribute(
                e,
                this.getAttribute(e) ? this.getAttribute(e) : ""
              )
            : this.textContent.removeAttribute(e)
          : "placeholder" === e
          ? (this.placeholder.textContent = this.getAttribute("placeholder"))
          : "value" === e &&
            ((this.textarea.value = n),
            (this.textareaBox.dataset.value = n),
            this.checkInput());
      }
    }
  );
  ...
  class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.shadowRoot.querySelector("#adblocker_warning").innerHTML = `
                    ...
                    ${
                      isBrave
                        ? `<strong>If you have enabled Brave shield then disable it also.</strong>`
                        : ""
                    }
                `;
    }
