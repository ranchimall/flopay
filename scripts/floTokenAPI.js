(function (EXPORTS) { //floTokenAPI v1.2.1a
    ...
    tokenAPI.sendToken = function (privKey, amount, receiverID, message = "", token = DEFAULT.currency, options = {}) {
        console.log("inside tokenAPI.sendToken")
        return new Promise((resolve, reject) => {
    ...
    tokenAPI.sendUsdtToken = function (privKey, amount, receiverID, message = "", options = {}) {
        console.log("inside tokenAPI sendUsdtToken");
        ...
        return new Promise((resolve, reject) => {
            ...
            // Get USDT balance and validate
            getBalance(senderID, "usdt").then(bal => {
                console.log("inside getBalance ")
                if (amount > bal) {
                    return reject("Insufficient USDT balance");
                }
    
                // Write USDT transaction data to the blockchain
                floBlockchainAPI.writeData(senderID, `send ${amount} USDT# ${message}`, privKey, receiverID, options).then(txid => resolve(txid)).catch(error => reject(error));
            }).catch(error => reject(error));
        });
    };
