(function (EXPORTS) { //ethOperator v1.0.2
  ...
  //     }
  // };
  const sendToken = ethOperator.sendToken = async ({ token, privateKey, amount, receiver, contractAddress, fees }) => {
    console.log("Inside sendToken");
    ...
    const requiredFeeWei = ethers.utils.parseEther(fees.toString());
    if (ethBalance.lt(requiredFeeWei)) {
        notify(`Insufficient ETH balance for gas fees. Required: ${ethers.utils.formatEther(requiredFeeWei)} ETH`);
    }

    // Perform transfer
    try {
        const tx = await tokenContract.transfer(receiver, amountWei, {
            gasLimit: 100000, // Optional: Replace with a dynamically calculated gas limit
            gasPrice: requiredFeeWei, // Use the fees parameter as gas price
        });
        console.log("Raw Transaction:", tx);

        if (!tx.hash) {
            notify("Transaction failed or tx.hash is undefined");
        }
